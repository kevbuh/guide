...
def apply_all_laws(expr, do_print=False):
  """
  ...
  for node in walk(parsed_code): 
      match node:
          case BoolOp(op=Or(), values=[a, Constant(value=0)]) | BoolOp(op=And(), values=[a, Constant(value=1)]):     
              ...
              if do_print: print(f" - Applying Identity Law: {expr} = {modified_code[:-1]}")
  ...
  for node in walk(parsed_code): 
      match node:
          case BoolOp(op=Or(), values=[a, Constant(value=1)]):                                                        
              ...
              if do_print: print(f" - Applying Domination Law: {expr} = {modified_code[:-1]}")
  ...
  for node in walk(parsed_code): 
      match node:
          case BoolOp(op=And(), values=[a, Constant(value=0)]):                                                       
              ...
              modified_code = astor.to_source(replaced_tree)
              new_expressions["Domination Law"].append(modified_code[:-1])
              if do_print: print(f" - Applying Domination Law 2: {expr} = {modified_code[:-1]}")
  ...
  for node in walk(parsed_code): 
      match node:
          case UnaryOp(op=Not(), operand=UnaryOp(op=Not(), operand=a)):                                               
              ...
              if do_print: print(f" - Applying Double Negation Law: {expr} = {modified_code[:-1]}")
  ...
  for node in walk(parsed_code): 
      match node:
          case BoolOp(op=Or(), values=[a, b]):                                                                        
              ...
              if do_print: print(f" - Applying Commutative Law: {expr} = {modified_code[:-1]}")
  ...
  for node in walk(parsed_code): 
      match node:
          case BoolOp(op=Or(), values=[BoolOp(op=Or(), values=[a, b]), c]):                                           
              ...
              if do_print: print(f" - Applying Associative Law: {expr} = {modified_code[:-1]}")
  ...
  for node in walk(parsed_code): 
      match node:
          case BoolOp(op=And(), values=[BoolOp(op=And(), values=[a, b]), c]):                                         
              ...
              modified_code = astor.to_source(replaced_tree)
              new_expressions["Associative Law"].append(modified_code[:-1])
              if do_print: print(f" - Applying Associative Law 2: {expr} = {modified_code[:-1]}")
  ...
  for node in walk(parsed_code): 
      match node:
          case BoolOp(op=And(), values=[a, BoolOp(op=Or(), values=[b, c])]):                                          
              ...
              if do_print: print(f" - Applying Distributive Law: {expr} = {modified_code[:-1]}")
      ...
      match node:
          case BoolOp(op=Or(), values=[a, BoolOp(op=And(), values=[*objects])]):                                      
              if t_util.are_subtrees_equivalent(a, objects[0]): 
                  ...
                  if do_print: print(f" - Applying Absorption Law: {expr} = {modified_code[:-1]}")
      ...
      match node:
          case BoolOp(op=Or(), values=[*objects]):                                                                    
              if t_util.are_subtrees_equivalent(objects[0], objects[1]):
                  ...
                  if do_print: print(f" - Applying Idempotent Law OR: {expr} = {modified_code[:-1]}")
      ...
      match node:
          case BoolOp(op=And(), values=[*objects]):                                                                    
              if t_util.are_subtrees_equivalent(objects[0], objects[1]):
                  ...
                  modified_code = astor.to_source(replaced_tree)
                  new_expressions["Idempotent Law"].append(modified_code[:-1])
                  if do_print: print(f" - Applying Idempotent Law AND: {expr} = {modified_code[:-1]}")
